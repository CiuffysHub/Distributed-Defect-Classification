...
dos2unix config/spark/spark-env.sh
dos2unix scripts/spark-services.sh
...
docker rmi mjaglan/ubuntuhadoop2017	
...
# start hadoop slave container(s), the first one with port mapping too
i=1

HADOOP_SLAVE="$HOST_PREFIX"-slave-$i
docker run --name $HADOOP_SLAVE -h $HADOOP_SLAVE --net=$NETWORK_NAME -p 8042:8042 -itd "$IMG_NAME"
i=$(( $i + 1 ))
...
docker run --name $HADOOP_MASTER -h $HADOOP_MASTER --net=$NETWORK_NAME \
		...
		-p  8080:8080  -p 8888:8888  -p 7077\
		-itd "$IMG_NAME"
